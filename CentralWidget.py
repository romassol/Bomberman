...
class CentralWidget(QWidget):
    ...
    def set_scoring(self):
        ...
        print(self.scoring)
        print(self.scoring.pos())

    def set_graphical_parameters(self):
        self.set_background(self.images_folder + "grass.png")
        self.bomb = CharacterImage(self, self.level.Bomb, self.size_square, self.images_folder + "bomb.png")
        self.bomb.hide()
        self.bombs = []
        for b in self.level.BomberMan:
            self.bomberman_image = CharacterImage(self, b, self.size_square, self.name_of_image_BM)
        ...
        self.monsters = []
        self.create_and_append_character(self.level.Monster, self.name_of_image_monster, self.monsters)
        self.lives = []
        self.draw_lives()
    ...
    def set_names_of_images(self):
        ...
        self.names_of_images_walls = {'False None': self.images_folder + 'metal_wall.png',
        ...
        self.names_of_images_prizes = {'1': self.images_folder + 'heart.png', '2': self.images_folder + 'bomb_2.png',
                                       '3': self.images_folder + 'bomb_3.png'}
        self.names_of_images_bombs = {1: self.images_folder + "bomb.png", 2: self.images_folder + 'bomb_2.png',
                                      3: self.images_folder + 'bomb_3.png'}

    def calculate_size_square(self):
        return int(min(self.height/self.level.height, self.width/self.level.width))

    def calculate_size_window(self):
        return (self.level.width * self.size_square, self.level.height * self.size_square)

    def create_and_append_walls(self):
        for w in self.level.Wall:
            self.walls.append(CharacterImage(self, w, self.size_square, self.names_of_images_walls[str(w)]))

    def create_and_append_prizes(self):
        for p in self.level.Prizes:
            self.prizes.append(CharacterImage(
                self, p, self.size_square, self.names_of_images_prizes[str(p).split(',')[1].split()[1]]))

    def create_and_append_character(self, input_characters_list, image, finaly_characters_list):
        for character in input_characters_list:
            finaly_characters_list.append(CharacterImage(self, character, self.size_square, image))

    def draw_lives(self):
        self.lives.clear()
        for i in range(self.bomberman_image.character.count_of_lives):
            position = Point(i, 0)
            icon = IconImage(self, position, self.size_square / 3, self.images_folder + 'heart.png')
            self.lives.append(icon)

    def set_background(self, image):
        pixmap = QPixmap(image)
        self.background.setPixmap(pixmap)
        self.background.setGeometry(0, 0, self.size_window[0], self.size_window[1])
    ...
    def show_game_over(self):
        ...
        go.setStyleSheet("QPushButton {border:none; background-color:transparent;}")
    ...
    def show_win(self):
        ...
        win.setStyleSheet("QPushButton {border:none; background-color:transparent;}")
    ...
    def try_change_level(self):
        ...
        self.size_square = self.calculate_size_square()
        self.size_window = self.calculate_size_window()
        self.hide_all_graphical_parameters([self.bombs, self.prizes, self.walls, self.monsters, self.lives])
        self.set_graphical_parameters()
        self.scoring.hide()
        self.scoring = None
        self.set_scoring()
    ...
    def timerEvent(self, e):
        ...
        if not self.level.is_BM_lives_changed:
            ...
            self.bomberman_image.set_picture(self.size_square, self.images_folder + "person_hurt.png")

        if not self.level.Bomb.is_active:
            self.timer_bomb = 1
        else:
            if self.timer_bomb % self.time_to_bomb == 0:
                self.boom()
        ...
        if self.game.is_win():
            self.show_win()
            player_name = QInputDialog.getText(self.parent, 'win', 'enter your name', text='anon')
            if player_name[1]:
                self.scoreboard.add_score(self.game.game_type, player_name[0], float(self.game.get_scoring()))
                self.score_window.show_current(self.game.game_type, player_name[0], float(self.game.get_scoring()))
            self.timer.stop()
        ...
        for p in self.prizes:
            if self.bomberman_image.position == p.position:
                ...
                if isinstance(p.character, PrizeLife):
                    self.paint_icon_and_add_prize(Point(len(self.lives), 0), self.lives,
                ...
                if isinstance(p.character, PrizeBomb):
                    ...
                    self.paint_icon_and_add_prize(Point(len(self.bombs), 1), self.bombs,
                                                  self.size_square/3, self.names_of_images_bombs[power])
        for b in removed:
