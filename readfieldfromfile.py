...
class Reader:
    def __init__(self, file_name):
        self.classes = {"B": BomberMan, "#": Wall, "*": Wall,
                        "U": Wall, "D": Wall, "R": Wall,
                        "L": Wall, "u": Wall, "d": Wall,
                        "r": Wall, "l": Wall, "M": Monster}
        self.walls = {"#": [False, None], "*": [True, None],
                      "U": [False, 'u'], "D": [False, 'd'],
                      "R": [False, 'r'], "L": [False, 'l'],
                      "u": [True, 'u'], "d": [True, 'd'],
                      "r": [True, 'r'], "l": [True, 'l']}
    ...
    def check_added_parameters(self, parameter, data):
        try:
            ...
            raise KeyError("you need to specify a " + parameter)
        ...
        for i in range(len(input_prizes)):
            for j in range(len(input_prizes[i])):
                ...
                if code != ".":
                    ...
                    self.objects['Prizes'].add(
                        self.prize[code](int(code), Point(j, i)))
    ...
    def read_file(self, file_name):
        ...
        if len(input_prizes) == 0:
            raise ValueError("you need to set a prizes")
        input_map = self.check_added_parameters('field', data)
        if len(input_map) == 0:
            raise ValueError("you need to set a field")
